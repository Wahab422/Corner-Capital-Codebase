 ...
 * - data-fade-ease="cubic-bezier(0.22, 1, 0.36, 1)": easing for fade mode (default ease-out).
 * - data-slide-duration="40": slide transition duration (Embla physics, 20–60; higher = slower). Ignored in fade mode.
 * - data-slide-ease="cubic-bezier(0.25, 0.1, 0.25, 1)": easing for slide mode; exposed as --carousel-slide-ease for CSS. Embla’s scroll still uses physics; use this for custom transitions on slide content.
 ...
 * - data-axis="y" or data-vertical: vertical (Y-axis) slider; default is horizontal (x).
 * - data-show-slides="2" | "3" | "4": number of slides visible in the viewport (sets flex basis on slides). Ignored in fade mode.
 * - data-arrow-step="2": number of slides to move when clicking next/prev buttons (default 1).
 *
 * Vertical slider markup / CSS:
 * - Use data-axis="y" or data-vertical on the carousel root.
 * - Give the viewport a fixed height (e.g. .carousel-wrapper { height: 400px; overflow: hidden; }).
 * - Make the track vertical: .carousel { display: flex; flex-direction: column; } (or leave default block; Embla handles it).
 */
...
function initializeCarousels(sliderList) {
  ...
  sliderList.forEach((slider) => {
    ...
    const slideDuration = Number.isFinite(Number.parseInt(durationAttr, 10))
      ? Math.max(20, Math.min(60, Number.parseInt(durationAttr, 10)))
      : 25;
    ...
    const showSlides =
      showSlidesAttr != null &&
      Number.isFinite(Number.parseFloat(showSlidesAttr)) &&
      Number.parseFloat(showSlidesAttr) >= 1
        ? Number.parseFloat(showSlidesAttr)
        : null;
    ...
    function applyShowSlidesLayout() {
      if (!showSlides || fadeMode || !carouselContainer) return;

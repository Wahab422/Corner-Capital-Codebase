...
/** Set by initVideoScript(); called by cleanupVideoPlayer() to remove document listeners and dispose players. */
let scriptTeardown = null;
...
export function cleanupVideoPlayer() {
  ...
  if (scriptTeardown) {
    try {
      ...
      handleError(err, 'Video Player Script Teardown');
    }
    scriptTeardown = null;
  }
...
:where(.vds-video-layout) {
  ...
  scriptTeardown = () => {
    ...
    if (readyIO) readyIO.disconnect();
    if (posterIO) posterIO.disconnect();
    if (posterFetchIO) posterFetchIO.disconnect();
    if (posterStartIO) posterStartIO.disconnect();
    playerMap.forEach((player) => {
      try {
        if (typeof player.destroy === 'function') player.destroy();
        else if (typeof player.dispose === 'function') player.dispose();
      } catch {
        // ignore
      }
    });

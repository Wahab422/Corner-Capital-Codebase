...
/**
 * In-flight load promises so concurrent loadLibrary(name) calls only load once per library
 */
 ...
 * Uses a shared in-flight promise per library so concurrent callers only load once.
...
export async function loadLibrary(libraryName, options = {}) {
  ...
  // Already loading: wait for the in-flight load
